<script>
    ...
    function checkAllTodos(event){
        ...
        todos = todos; //must notify svelte that a change has occurred
    }
    ...
    function handleDeleteTodo(event){
        todo = todos.filter(todo => todo.id !== event.detail.id);
    }
...
<div class="container">
    <img src={'/img/oas-card-front.png'} class="app-image" alt="Orange Alligator Solutions Logo">

    <h2>Todos</h2>
    <input type="text" class="todo-input" placeholder="Insert new todo item..." 
        bind:value={newTodoTitle} on:keydown={addTodo}>

    {#each filteredTodos as todo}  
        <div class="todo-item">
            <TodoItem {...todo} on:deleteTodo={handleDeleteTodo} on:toggleComplete={handleToggleComplete}></TodoItem>
        </div>
    {/each}  
